...
@section('title', trans('t.all_obligatory-catalogues'))

@section('content')

    <div class="element-actions">
        <a class="btn btn-primary btn-sm" href="{{ route('admin.obligatory-catalogue.create') }}"><i class="os-icon os-icon-ui-22"></i><span>{{trans('t.create_obligatory-catalogue')}}</span></a>
    </div>
    ...
    <div class="element-box">
        <div class="table-responsive">
            <table id="datatables" class="dataTables_wrapper table-sm" cellspacing="0" width="100%">
                ...
                <tr>
                    <th>ID</th>
                    <th>{{trans('t.placeholder_module')}}</th>
                    <th>{{trans('t.obligatory_module')}}</th>
                    <th>{{trans('t.nr')}}</th>
                    <th>{{trans('t.ects')}}</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#datatables tbody').on( 'click', 'tr', function (el) {
                ...
                window.location.href = "obligatory-catalogue/"+ data[0];
            });
        });
    </script>
